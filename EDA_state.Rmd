---
title: "Exploratory Data Analysis for One State"
...
```{r}
state_data <- read_rds(file.choose())

state.data <- state_data %>% mutate(year= year(forecast_target))
```
...
state.data %>% 
  ...
  scale_color_viridis() +
  transition_states(year,state_length = 5,transition_length = 8) +
