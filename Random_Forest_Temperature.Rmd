...
```{r}
hindcast_data <- readRDS("./data/hindcast_all.rds")#look at random sample of data
hindcast_data <- hindcast_data %>%
  mutate(unique_row_identifer = rownames(hindcast_data)) ##maybe delete these lines and put them in the data processing file
rownames(hindcast_data) <- seq(1,nrow(hindcast_data),1)
```
...
```{r}
test.data.1 <- readRDS(file.choose())
test.data.2<- readRDS(file.choose())

test.data <- rbind(test.data.1,test.data.2)
test.data$forecast_timestamp <- as.numeric(test.data$forecast_timestamp)


test.data.mutate <- test.data %>% 
  mutate(forecast_target = as.Date(paste0(forecast_target, "01"),
                                    format = "%Y%m%d",
                                    tz = "EST"))

test.data.mutate %>% 
  filter(forecast_target >= "1983-02-01" )
...
training.data <- hindcast_data %>% 
  filter(forecast_timestamp < 2007010100)

testing.data <- hindcast_data[-as.numeric(rownames(training.data)),]
```
